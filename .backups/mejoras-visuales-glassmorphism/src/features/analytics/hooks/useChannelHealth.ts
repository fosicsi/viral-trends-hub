    ...
    return useMemo(() => {
        if (loading) {
            return {
                ...
                diversity: { score: 0, topVideoShare: 0, label: 'Cargando...', color: 'text-muted-foreground' },
        ...
        if (!reportData?.rows || !recentVideos?.length) {
            return {
                ...
                diversity: { score: 0, topVideoShare: 0, label: 'Sin datos', color: 'text-slate-400 bg-slate-400/10' },
