        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-slate-950 rounded-[24px] shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col md:flex-row border border-slate-200 dark:border-slate-800">
                ...
                <div className="flex-1 overflow-y-auto p-6 md:p-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 opacity-60">
                        {/* Outlier score (Locked) */}
                        <div className="border border-slate-200 dark:border-slate-800 rounded-xl p-5 relative">
                            <h3 className="font-semibold text-sm mb-4 flex items-center gap-2">Outlier score <Lock className="w-3.5 h-3.5 text-slate-400" /></h3>
                            <div className="relative pt-6 pb-2">
                                <div className="w-full h-1.5 bg-slate-200 dark:bg-slate-800 rounded-lg"></div>
                                <div className="flex justify-between text-xs text-slate-500 mt-3 font-medium">
                                    <span>0x</span>
                                    <span>&gt;1000x</span>
                                </div>
                        ...
                        {/* Subscriber count (Locked) */}
                        <div className="border border-slate-200 dark:border-slate-800 rounded-xl p-5 relative">
                            <h3 className="font-semibold text-sm mb-4 flex items-center gap-2">Subscriber count <Lock className="w-3.5 h-3.5 text-slate-400" /></h3>
                            <div className="relative pt-6 pb-2">
                                <div className="w-full h-1.5 bg-slate-200 dark:bg-slate-800 rounded-lg"></div>
                                <div className="flex justify-between text-xs text-slate-500 mt-3 font-medium">
                                    <span>0</span>
                    ...
                    {/* Upload date (Locked) */}
                    <div className="border border-slate-200 dark:border-slate-800 rounded-xl p-5 mb-6 opacity-60">
                        <h3 className="font-semibold text-sm mb-4 flex items-center gap-2">Upload date <Lock className="w-3.5 h-3.5 text-slate-400" /></h3>
                        <div className="flex flex-wrap gap-2">
                            {["This week", "This month", "Last 3 months", "Last 6 months", "This year", "Last 2 years", "Last 3 years"].map((label, i) => (
                                <div key={label} className={`px-4 py-2 rounded-lg text-xs font-medium ${i === 6 ? 'bg-primary/20 text-primary' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'}`}>
                                    {label}
                                </div>
                            ))}
                        </div>
                    ...
                    {/* Language (Locked) */}
                    <div className="border border-slate-200 dark:border-slate-800 rounded-xl p-5 mb-6 opacity-60">
                        <h3 className="font-semibold text-sm mb-4 flex items-center gap-2">Language <Lock className="w-3.5 h-3.5 text-slate-400" /></h3>
                        <div className="flex flex-wrap gap-2">
                            {["All", "English", "German", "Spanish", "French", "Amharic", "Arabic", "Assamese", "Bangla", "Bulgarian", "Marathi", "Catalan", "Cebuano", "Chinese", "+36 more"].map((label, i) => (
                                <div key={label} className={`px-4 py-2 rounded-lg text-xs font-medium ${i === 0 ? 'bg-primary/20 text-primary' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'}`}>
                                    {label}
                                </div>
                            ))}
                        </div>
