...
export function ScriptDisplayModal({ isOpen, onClose, scriptData, videoTitle }: ScriptDisplayModalProps) {
    ...
    // Detect if it's an AI Idea (no 'script' property but has 'niche' or 'reasoning')
    const isIdea = !scriptData.script && (scriptData.niche || scriptData.reasoning);
        ...
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto rounded-3xl border-border bg-card shadow-2xl p-0 gap-0">
                ...
                <div className="bg-gradient-to-r from-primary/10 to-purple-500/10 p-6 border-b border-border">
                    <div className="flex items-start justify-between gap-4">
                        <div>
                            <DialogTitle className="text-2xl font-black tracking-tight flex items-center gap-2">
                                ...
                                {isIdea ? "Idea de Contenido (IA)" : "Guion Viral Generado"}
                            </DialogTitle>
                            <DialogDescription className="text-muted-foreground mt-1">
                                {isIdea ? "Estrategia recomendada basada en datos." : "Basado en análisis de datos y comentarios de audiencia para:"} <span className="font-bold text-foreground">{videoTitle}</span>
                            </DialogDescription>
                        </div>
                    ...
                    {/* Key Insight Summary */}
                    {(scriptData.analysis || scriptData.reasoning) && (
                        <div className="mt-4 bg-background/60 backdrop-blur-sm rounded-xl p-3 border border-border/50 flex gap-3 text-sm">
                            ...
                            <div>
                                ...
                                <p className="text-muted-foreground leading-snug">
                                    {isIdea ? scriptData.reasoning : (scriptData.analysis?.content_gap || scriptData.analysis?.angle)}
                                </p>
                            </div>
                        </div>
                    )}
                ...
                <div className="p-6">
                    {/* VIEW FOR IDEAS (PRE-SCRIPT) */}
                    {isIdea ? (
                        <div className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-4">
                                    <h4 className="font-bold text-lg flex items-center gap-2"><Check className="w-5 h-5 text-green-500" /> Por qué funcionará</h4>
                                    <div className="bg-muted/30 p-4 rounded-xl border border-border">
                                        <p className="text-sm leading-relaxed">{scriptData.reasoning}</p>
                                    </div>

                                    <h4 className="font-bold text-lg flex items-center gap-2 mt-6"><Search className="w-5 h-5 text-blue-500" /> Estrategia de Retención</h4>
                                    <div className="bg-muted/30 p-4 rounded-xl border border-border">
                                        <p className="text-sm leading-relaxed">{scriptData.retentionStrategy}</p>
                                    </div>
                                </div>

                                <div className="space-y-4">
                                    <h4 className="font-bold text-lg">Títulos Sugeridos</h4>
                                    <div className="space-y-2">
                                        {scriptData.titleSuggestions?.map((title: string, i: number) => (
                                            <div key={i} className="p-3 bg-card border border-border rounded-lg text-sm font-medium flex justify-between items-center group">
                                                {title}
                                                <Button size="icon" variant="ghost" className="h-6 w-6 opacity-0 group-hover:opacity-100" onClick={() => handleCopy(title, `t-${i}`)}>
                                                    {copiedSection === `t-${i}` ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                                                </Button>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div className="pt-4">
                                        <div className="bg-purple-50 text-purple-900 dark:bg-purple-900/20 dark:text-purple-100 p-4 rounded-xl text-sm font-medium text-center">
                                            ⏰ <strong>Mejor hora para publicar:</strong> {scriptData.bestTimeToPost}
                                        </div>
                                    </div>
                                </div>
                            </div>
