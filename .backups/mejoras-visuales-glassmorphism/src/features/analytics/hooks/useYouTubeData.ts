        ...
        const checkConnection = async () => {
            ...
            try {
                ...
                try {
                    ...
                    if (apiError.message === 'AUTH_REVOKED') {
                        console.warn("[YouTubeData] Auth revoked, forcing disconnect.");
                        setData(prev => ({ ...prev, isConnected: false, loading: false, error: "Tu conexión expiró o fue revocada. Por favor, vuelve a conectar tu cuenta." }));
                        return;
                    }
                ...
                if (statsData && statsData.stats) {
                    console.log("[YouTubeData] Stats fetched:", statsData.stats.title, "ID:", (statsData.stats as any).id);
                    if (statsData.debug) console.log("[YouTubeData] API Debug Info:", statsData.debug);
                    ...
                    setData(prev => ({
                        ...
                        channelTitle: statsData.stats.title,
                        channelThumbnail: statsData.stats.thumbnail
                    }));
                }
            } catch (err: any) {
    ...
    useEffect(() => {
        const fetchReport = async () => {
            ...
            try {
                ...
                if (e.message === 'AUTH_REVOKED') {
                    setData(prev => ({ ...prev, isConnected: false, error: "Tu conexión expiró o fue revocada. Por favor, vuelve a conectar tu cuenta." }));
