...
Deno.serve(async (req) => {
    ...
    try {
        ...
        if (action === 'init') {
            ...
            // Force consent and offline access to guarantee a refresh token
            const finalPrompt = prompt.includes('consent') ? prompt : `${prompt} consent`;

            const authUrl = `${GOOGLE_AUTH_URL}?client_id=${clientId}&redirect_uri=${encodeURIComponent(finalRedirectUri)}&response_type=code&scope=${encodeURIComponent(scopes)}&access_type=offline&prompt=${encodeURIComponent(finalPrompt)}&state=${state}`;
        ...
        if (action === 'exchange') {
            ...
            // Preserve existing metadata (like channel_id)
            const { data: existingIntegration } = await supabaseAdmin
                .from('user_integrations')
                .select('metadata, refresh_token')
                .eq('user_id', user.id)
            ...
            const payload: any = {
                ...
                metadata: existingIntegration?.metadata || {},
            ...
            if (encryptedRefreshToken) {
                ...
                // Keep the old refresh_token if Google didn't send a new one
                payload.refresh_token = existingIntegration.refresh_token;
            }
        ...
        if (action === 'stats') {
            ...
            const { data: integration, error: dbError } = await supabaseAdmin
                .from('user_integrations')
                .select('access_token, refresh_token, expires_at, platform, metadata')
                .eq('user_id', user.id)
            ...
            if (expiresAt.getTime() - now.getTime() < 300 * 1000) {
                ...
                if (refreshData.error === 'invalid_grant') {
                    return new Response(JSON.stringify({ error: "AUTH_REVOKED", message: "Token expired or revoked." }), { status: 401, headers: { ...dynamicCorsHeaders, 'Content-Type': 'application/json' } });
                }
            ...
            try {
                ...
                if (ytRes.status === 401) {
                    // Possible race condition: another call refreshed the token. Retry once with fresh DB read.
                    console.log(`[stats] 401 detected, retrying with fresh integration from DB...`);
                    const { data: freshInt } = await supabaseAdmin.from('user_integrations').select('access_token, refresh_token, metadata').eq('user_id', user.id).in('platform', [platform, 'google', 'youtube']).order('created_at', { ascending: false }).limit(1).single();
                    if (freshInt && freshInt.access_token !== integration.access_token) {
                        const freshToken = await decrypt(freshInt.access_token, encryptionKey);
                        const retryRes = await fetch(`https://www.googleapis.com/youtube/v3/channels?part=statistics,snippet&mine=true`, { headers: { Authorization: `Bearer ${freshToken}` } });
                        if (retryRes.ok) {
                            const retryData = await retryRes.json();
                            // If successful, proceed with retryData
                            const channel = retryData.items[0];
                            const stats = {
                                id: channel.id, title: channel.snippet.title, description: channel.snippet.description, thumbnail: channel.snippet.thumbnails?.default?.url,
                                viewCount: channel.statistics.viewCount, subscriberCount: channel.statistics.subscriberCount, videoCount: channel.statistics.videoCount, hiddenSubscriberCount: channel.statistics.hiddenSubscriberCount
                            };
                            return new Response(JSON.stringify({ stats, cached: false, fetchedAt: now.toISOString(), debug: { retried: true, channelId: channel.id } }), { headers: { ...dynamicCorsHeaders, 'Content-Type': 'application/json' } });
                        }
                    }
                }
                ...
                if (!ytRes.ok) {
                    ...
                    throw new Error(`YouTube API Error: ${ytRes.status} ${ytData.error?.message || ""}`);
                }
                ...
                // PERSIST CHANNEL ID IN METADATA if not already there
                const integrationMetadata = (integration.metadata as any) || {};
                if (!integrationMetadata.channel_id) {
                    console.log(`[stats] Updating integration metadata with channel_id: ${channel.id}`);
                    await supabaseAdmin.from('user_integrations').update({
                        metadata: { ...integrationMetadata, channel_id: channel.id }
                ...
                return new Response(JSON.stringify({
                    ...
                    debug: {
                        channelCount: ytData.items.length,
                        primaryChannelId: channel.id,
                        primaryChannelTitle: channel.snippet.title,
                        allChannelTitles: ytData.items.map((i: any) => i.snippet.title)
                    }
                }), { headers: { ...dynamicCorsHeaders, 'Content-Type': 'application/json' } });
        ...
        if (action === 'videos') {
            ...
            const cacheKey = `v3_videos_${maxResults}_${order}`;
            ...
            try {
                ...
                const channelId = (integration.metadata as any)?.channel_id;
                const channelParam = channelId ? `id=${channelId}` : `mine=true`;

                // Try fetching Channel ID/Uploads first (High Cost: 1 unit)
                const channelRes = await fetch(`https://www.googleapis.com/youtube/v3/channels?part=id,contentDetails&${channelParam}`, { headers: { Authorization: `Bearer ${accessToken}` } });
                ...
                if (channelRes.ok) {
                    ...
                    console.log(`[videos] 401 on channel fetch, retrying with fresh integration...`);
                    const { data: freshInt } = await supabaseAdmin.from('user_integrations').select('access_token, refresh_token, metadata').eq('user_id', user.id).in('platform', [platform, 'google', 'youtube']).order('created_at', { ascending: false }).limit(1).single();
                    if (freshInt && freshInt.access_token !== integration.access_token) {
                        accessToken = await decrypt(freshInt.access_token, encryptionKey);
                        // Retry the channel fetch with the new token
                        const retryRes = await fetch(`https://www.googleapis.com/youtube/v3/channels?part=id,contentDetails&${channelParam}`, { headers: { Authorization: `Bearer ${accessToken}` } });
                        if (retryRes.ok) {
                            const retryData = await retryRes.json();
                            uploadsPlaylistId = retryData.items?.[0]?.contentDetails?.relatedPlaylists?.uploads;
                        } else {
                            throw new Error(`Channel fetch retry failed: ${retryRes.status}`);
                        }
                    } else if (integration.refresh_token) {
                        console.log(`[videos] Token same in DB, forcing a refresh now...`);
                        const refreshToken = await decrypt(integration.refresh_token, encryptionKey);
                        ...
                        if (refreshData.access_token) {
                            ...
                            // FINAL RETRY
                            const finalRes = await fetch(`https://www.googleapis.com/youtube/v3/channels?part=id,contentDetails&${channelParam}`, { headers: { Authorization: `Bearer ${accessToken}` } });
                            if (finalRes.ok) {
                                const finalData = await finalRes.json();
                                uploadsPlaylistId = finalData.items?.[0]?.contentDetails?.relatedPlaylists?.uploads;
                            } else {
                                throw new Error(`Channel fetch failed even after force refresh: ${finalRes.status}`);
                            }
                        } else {
                            if (refreshData.error === 'invalid_grant') {
                                return new Response(JSON.stringify({ error: "AUTH_REVOKED", message: "Token expired or revoked." }), { status: 401, headers: { ...dynamicCorsHeaders, 'Content-Type': 'application/json' } });
                            }
                            throw new Error(`Channel fetch failed and refresh failed: ${JSON.stringify(refreshData)}`);
                        }
                    } else {
                        throw new Error(`Channel fetch failed: 401 (No refresh token available)`);
                    }
                } else if (!channelRes.ok) {
                    throw new Error(`Channel fetch failed: ${channelRes.status}`);
                }

                if (!uploadsPlaylistId) throw new Error("No uploads playlist ID found for this channel");

                // Get Playlist Items (Fetch more than requested to allow for filtering non-public/queued videos)
                const fetchCount = Math.max(maxResults * 3, 20);
                const videosRes = await fetch(`https://www.googleapis.com/youtube/v3/playlistItems?part=snippet,contentDetails&playlistId=${uploadsPlaylistId}&maxResults=${fetchCount}`, { headers: { Authorization: `Bearer ${accessToken}` } });

                if (!videosRes.ok) {
                    const errorData = await videosRes.json().catch(() => ({}));
                    console.error("[videos] PlaylistItems Error:", errorData);
                    ...
                    throw new Error(`Playlist fetch failed: ${videosRes.status} ${errorData.error?.message || ""}`);
                }

                const videosData = await videosRes.json();
                if (!videosData.items || videosData.items.length === 0) {
                    return new Response(JSON.stringify({ videos: [], cached: false, fetchedAt: now.toISOString(), message: "No videos found in uploads playlist" }), { headers: { ...dynamicCorsHeaders, 'Content-Type': 'application/json' } });
                }

                const videoIds = videosData.items.map((item: any) => item.contentDetails.videoId).filter(Boolean).join(',');

                // Get Video Details (including status to filter out non-public/scheduled videos)
                const detailsRes = await fetch(`https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails,statistics,status&id=${videoIds}`, { headers: { Authorization: `Bearer ${accessToken}` } });

                if (!detailsRes.ok) {
                    const errorData = await detailsRes.json().catch(() => ({}));
                    console.error("[videos] VideoDetails Error:", errorData);
                    throw new Error(`Video details fetch failed: ${detailsRes.status}`);
                }

                const detailsData = await detailsRes.json();

                // FILTER AND SORT: 
                // 1. Only public AND processed
                // 2. Not upcoming (Premieres in queue)
                // 3. Not in the future (Scheduled)
                // 4. Sort by publishedAt DESC
                const publicVideos = (detailsData.items || [])
                    .filter((v: any) =>
                        v.status?.privacyStatus === 'public' &&
                        v.status?.uploadStatus === 'processed' &&
                        v.snippet?.liveBroadcastContent !== 'upcoming' &&
                        new Date(v.snippet?.publishedAt).getTime() <= now.getTime()
                    )
                    .sort((a: any, b: any) => new Date(b.snippet.publishedAt).getTime() - new Date(a.snippet.publishedAt).getTime())
                    .slice(0, maxResults);

                console.log(`[videos] Filtered ${detailsData.items?.length || 0} down to ${publicVideos.length} public/processed videos (v3)`);

                // Cache success (Short cache TTL for videos to keep it fresh)
                await supabaseAdmin.from('youtube_analytics_cache').upsert({
                    user_id: user.id, data_type: cacheKey, date_range: null, data: publicVideos, fetched_at: now.toISOString(), expires_at: new Date(now.getTime() + 15 * 60 * 1000).toISOString()
                }, { onConflict: 'user_id,data_type,date_range' });

                return new Response(JSON.stringify({ videos: publicVideos, cached: false, fetchedAt: now.toISOString() }), { headers: { ...dynamicCorsHeaders, 'Content-Type': 'application/json' } });

            } catch (err) {
                const errorMessage = err instanceof Error ? err.message : String(err);
                console.error("[videos] Critical Failure:", errorMessage, err);

                const { data: lastResort } = await supabaseAdmin.from('youtube_analytics_cache')
                    .select('data, fetched_at')
                    .eq('user_id', user.id)
                    .eq('data_type', cacheKey)
                ...
                if (lastResort) {
                    return new Response(JSON.stringify({
                        videos: lastResort.data,
                        cached: true,
                        isStale: true,
                        errorFallback: true,
                        details: errorMessage,
                        fetchedAt: lastResort.fetched_at
                    }), { headers: { ...dynamicCorsHeaders, 'Content-Type': 'application/json' } });
                }

                return new Response(JSON.stringify({
                    error: "Failed and no cache available",
                    details: errorMessage,
        ...
        if (action === 'reports' || action === 'get_reports') {
            ...
            // Fetch Integration
            const { data: integration } = await supabaseAdmin.from('user_integrations').select('access_token, refresh_token, expires_at, platform, metadata').eq('user_id', user.id).in('platform', [platform, 'google', 'youtube']).order('updated_at', { ascending: false }).limit(1).single();
            ...
            const channelId = (integration.metadata as any)?.channel_id || 'MINE';
            const queryParams = new URLSearchParams({
                ids: channelId.startsWith('UC') ? `channel==${channelId}` : 'channel==MINE', startDate: canonical.start, endDate: canonical.end, metrics: rMetrics, dimensions: rDims, sort: sort
            });
